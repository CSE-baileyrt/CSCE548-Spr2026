...
namespace Common
{
    /*
     * PROBLEMS:
     * #1: SqlConnection is in System.Data.SqlClient, which is obsolete
     * The new version is Microsoft.Data.SqlClient, but that doesn't have SqlConnection or SqlCommand
     * That's why SqlConnection and SqlCommand appear crossed out below (obsolete)
     * 
     * #2: This class uses SQL Server. My database server is MySQL
     */

    public class DataManagerV1
    {
        private readonly string _connectionString;

        public DataManagerV1(string connectionString)
        {
        ...
        public int CreateVideo(Video video)
        {
            ...
            using var cmd = new SqlCommand(
              ...
              OUTPUT INSERTED.Id
              VALUES (@Planned, @Recorded, @Posted, @Title, @Description, @Thumbnail)", conn);
        ...
        {
            using var conn = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand("SELECT * FROM Videos WHERE Id = @Id", conn);
        ...
        public int CreateLesson(Lesson lesson, int videoId)
        {
            ...
            using var cmd = new SqlCommand(
              ...
              OUTPUT INSERTED.Id
              VALUES (@VideoId, @Topic, @Summary, @Intro, @Module, @Sequence)", conn);
        ...
        public int CreateSlide(Slide slide)
        {
            ...
            using var cmd = new SqlCommand(
              ...
              OUTPUT INSERTED.Id
              VALUES (@LessonId, @Title, @Content, @Graphic, @Script)", conn);
        ...
        public List<Video> GetVideosByFlag(string flagName, bool value)
        {
            var videos = new List<Video>();
            using var conn = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand(
                $"SELECT * FROM Videos WHERE {flagName} = @Value", conn);
